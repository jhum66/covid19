        ...
        for row in reader:
            if first_row:
                ...
                if len (row_header) == 6:
                    ...
                    county          = str(validate_data(row [9], ''))
                    state           = str(validate_data(row [0], ''))
                    country         = str(validate_data(row [1], ''))

                    lat             = float(validate_data(row [3]))
                    long_           = float(validate_data(row [4]))
                    confirmed       = int(validate_data(row [5]))
                    deaths          = int(validate_data(row [6]))
                    recovered       = int(validate_data(row [7]))
                    active          = int(validate_data(row [8]))
...
def main():
    ...
    load_data ("../COVID-19-web-data/data/cases.csv", datetime.now())
