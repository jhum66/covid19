...
def main():
    ...
    rows = g_mydb.query_fetch_all("SELECT c.name, d.update_dt, f.confirmed_cases, f.deaths, f.recovered_cases FROM ts_rate2 f inner join d_datetime d on f.d_datetime_id = d.d_datetime_id inner join d_country c on f.d_country_id = c.d_country_id where f.d_country_id = 3 order by d.update_dt asc")
